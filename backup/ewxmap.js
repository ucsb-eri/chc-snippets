var EwxMap = function (configuration) {
  ...
  if (this.config.periodicity === '1-dekad') {
    ...
    this.config.period.temporal1 = padDateNumber(((this.config.period.month - 1) * 31) + this.config.period.day);
  }
  ...
  this.calculateTitle = function () {
    ...
    if (this.config.title === true) {
      ...
      if (periodicity == '1-month' || periodicity == '2-month' || periodicity == '3-month') {
        ...
        var monthAndDay = this.doyToMonthAndDay(this.config.period.year, this.config.period.day);
...
var TimeNavigator = function (series) {
  ...
  this.getNextDayGeoServerName = function (name) {
    var tokens = this.tokenizeGeoServerName(name);
    nextDay = Number(tokens.temporal1) + 1
    tokens.temporal1 = String(nextDay);
    tokens.year = String(tokens.year)
  ...
  this.getPreviousDayGeoServerName = function (name) {
    var tokens = this.tokenizeGeoServerName(name);
    prevDay = Number(tokens.temporal1) - 1
    // var doy = Number(tokens.temporal1);
    // var nextDoy = doy > 1 ? dekad - 1 : 36;
    //tokens.temporal1 = (prevDay < 10 ? '0' : '') + String(prevDay);
    tokens.temporal1 = String(prevDay);
