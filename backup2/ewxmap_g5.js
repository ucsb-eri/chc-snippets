var EwxMapG5 = function(configuration) {
  ...
  var sub_dataset = this.config.sub_dataset;
  console.log("sub_dataset; ", sub_dataset);
    ...
    $(function() {
      if (!window.MAP_STYLES_LOADING && !window.MAP_STYLES_LOADED) {
        ...
        $("<link/>", {
          rel: "stylesheet",
          type: "text/css",
          href: "file:///Users/marty/Projects/EWX/snippets/Snippets/styles.css"
          // href: "styles.css" mfl
        }).appendTo("head");
  ...
  this.loadEwxConfig = function(done) {
    ...
    if (this.config.period == 'latest') {
      ...
      $.ajax({
        ...
        console.log('ewx_config: ', ewx_config.ARC2);
        ...
        dataItemName = _this.config.sub_dataset.toLowerCase() + '_' + _this.config.region + '_' + periodicity + '_' + _this.config.statistic
...
*/
  ...
  this.changeTemporalIndex = function(evt) {
    ...
    if ($(evt.target).hasClass('previous-year-button')) {
      ...
      console.log('startYear: ', year, startYear);
      if (newYear >= startYear) {
        wmstTime = String(newYear) + '-' + timeArr[1] + '-' + timeArr[2];
        console.log('wmstTime: ', wmstTime);
      } else {
        console.log('reached startYear, wmstTime: ', wmstTime);
      }
  ...
  this.updateRasterLayer = function(tokens) { // does not appear to be called anywhere
    ...
    this.config.sub_dataset = tokens.sub_dataset;
